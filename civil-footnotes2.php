<?php
/*
Plugin Name: Civil Footnotes 2
Plugin URI: https://github.com/copernicus365/Wordpress-Civil-Footnotes-2
Version: 1.5.0
Description: Parses and displays footnotes, based on <a href="https://wordpress.org/plugins/civil-footnotes/">Civil Footnotes</a>, which itself was based on <a href="https://elvery.net/drzax/wordpress-footnotes-plugin">WP-Foonotes</a> by <a href="http://elvery.net">Simon Elvery</a>, and the footnote syntax pioneered by <a href="http://daringfireball.net/2005/07/footnotes">John Gruber</a>. The improvement here is that the title information is placed in a data-title attribute, not in the anchor's title attribute. This allows a custom CSS to style the popup, instead of the ugly default browser way. Without this, adding custom CSS worked, but the default popup still showed at the same time. See <a href="https://stackoverflow.com/a/35141804/">here</a> and <a href="https://stackoverflow.com/a/2011199/">here</a> for those issues, and for sample CSS styling. The downside about this way is you will have to add that styling somewhere in your stylesheet or wherever to see the popup, but that was the whole point, the default popup couldn't be hidden otherwise. I have also added a `civil-footnote` class value to the anchors, before the only way to select them was with 'a[rel="footnote"]', which works, but having a selector by class seems most appropriate, namely: `a.civil-footnote`, or in jQuery: `$('a.civil-footnote')`. Following is a *minimized* sample css, for a readable version see the readme, or unminize this somewhere, etc. <pre>`a.civil-footnote{display:inline}a.civil-footnote:hover{text-decoration-line:none}a.civil-footnote:hover:after{content:attr(data-title);z-index:99;font-size:18px;border-radius:10px;padding:4px 10px;margin-left:5px;color:#fff;background:rgba(55,55,55,.5)}</pre>
...
define('WP_FOOTNOTES_VERSION', '1.5.0');
...
class swas_wp_footnotes {
	...
	function process($data) {
		...
		foreach ($identifiers as $key => $value) {
			...
			$id_replace = '<sup id="'.$id_id.'"><a href="'.$id_href.'" data-title="'.$id_title.'" class="civil-footnote" rel="footnote">'.$id_num.'</a></sup>';
	...
	// Display the footnotes (here is where you can change the output)
		...
		// Create each footnote
			...
			$datanote = $datanote.' class="backlink" data-title="Jump back to footnote '.$id_num.' in the text.">'; // You can change the class or hover text
